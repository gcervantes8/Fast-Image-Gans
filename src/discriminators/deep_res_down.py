...
class DeepResDown(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, pooling=True):
        ...
        self.conv_b2 = spectral_norm(nn.Conv2d(in_channels, int(in_channels/4), kernel_size=1, padding='same'), eps=1e-04)
        nn.init.orthogonal_(self.conv_b2.weight)
        self.relu_b3 = nn.ReLU()
        self.conv_b4 = spectral_norm(nn.Conv2d(int(in_channels/4), int(in_channels/4), kernel_size=3, padding='same'), eps=1e-04)
        nn.init.orthogonal_(self.conv_b4.weight)
        self.relu_b5 = nn.ReLU()
        self.conv_b6 = spectral_norm(nn.Conv2d(int(in_channels/4), int(in_channels/4), kernel_size=3, padding='same'), eps=1e-04)
        nn.init.orthogonal_(self.conv_b6.weight)
        self.relu_b7 = nn.ReLU()
        self.conv_b9 = spectral_norm(nn.Conv2d(int(in_channels/4), out_channels, kernel_size=1, padding='same'), eps=1e-04)
        ...
        if not self.same_dim:
            self.conv_a2 = spectral_norm(nn.Conv2d(in_channels, out_channels-in_channels, kernel_size=1, padding='same'), eps=1e-04)
