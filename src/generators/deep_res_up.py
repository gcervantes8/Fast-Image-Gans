...
class DeepResUp(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, latent_embed_vector_size, upsample=True):
        ...
        self.conv_b7 = spectral_norm(nn.Conv2d(int(in_channels/4), out_channels, kernel_size=3, padding='same', bias=False), eps=1e-04)
        nn.init.orthogonal_(self.conv_b7.weight)
        self.cond_batch_norm_b8 = ConditionalBatchNorm2d(out_channels, latent_embed_vector_size)
        self.relu_b9 = nn.ReLU()
        self.conv_b10 = spectral_norm(nn.Conv2d(out_channels, out_channels, kernel_size=3, padding='same', bias=False), eps=1e-04)
        nn.init.orthogonal_(self.conv_b10.weight)
        self.cond_batch_norm_b11 = ConditionalBatchNorm2d(out_channels, latent_embed_vector_size)
