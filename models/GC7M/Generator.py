...
class Generator(nn.Module):
    def __init__(self, num_gpu, latent_vector_size, ngf, num_channels):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(latent_vector_size, ngf * 8, kernel_size=(4, 5), stride=1, padding=0, bias=False),
            nn.BatchNorm2d(ngf * 8),
            nn.ReLU(True),
            # state size: (ngf*8) x 4 x 5
            nn.ConvTranspose2d(ngf * 8, ngf * 4, kernel_size=(4, 5), stride=2, padding=1, bias=False),
            ...
            nn.ConvTranspose2d(ngf * 2, ngf, kernel_size=(5, 4), stride=2, padding=1, bias=False),
            nn.BatchNorm2d(ngf),
            nn.ReLU(True),
